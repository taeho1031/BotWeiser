...
def extract_info_from_bot_file(file_path):
    ...
    return name,timestamp, chain_ids

# Function to update the test_data.json file
def update_test_data(bot_id, name, timestamp, chain_ids):
    file_path = r'M:\FortaInfinity-BotWeiser\data\bots copy.json'
...
bot_folder = r'M:\FortaInfinity-BotWeiser\data\bots_date_chain_IDs'
...
for filename in os.listdir(bot_folder):
    if filename.endswith('.json'):
        ...
        name, timestamp, chain_ids = extract_info_from_bot_file(bot_file_path)

        # Update the test_data.json file with the extracted information
        update_test_data(bot_id, name, timestamp, chain_ids)
